...
function renderEnergyLive() {
    if (page !== 'energy' || !latestEnergy || !Campus.backendReady) return;
...
if (page === 'energy') {
    shell('Energy Analytics', 'Campus 25 live energy performance.', `<div class="page-heading"><div><h2>Energy Overview</h2><p class="muted">Live backend data. Historical daily/hourly data is not available.</p></div><span id="energyStatus" class="sub">● CONNECTING</span></div><div class="kpis energy-kpis">${[["Today's Consumption", 'kpiConsumption', 'Backend energy today'], ['Current Live Load', 'kpiCurrent', 'Current room power'], ['Energy Saved', 'kpiSaved', 'Backend automation savings'], ['Estimated Cost', 'kpiCost', 'Estimated at ₹8/kWh'], ['CO2 Reduction', 'kpiCo2', `Estimate at ${CO2_FACTOR} kg/kWh`]].map((item, index) => `<div class="card energy-kpi kpi-${index + 1}"><div class="kpi-head"><span>${item[0]}</span><span class="energy-dot"></span></div><div id="${item[1]}" class="value">Loading...</div><div class="sub">${item[2]}</div></div>`).join('')}</div><div class="charts energy-charts">${[['Current Room Power Consumption', 'daily', 'kW'], ['Power Consumption by Block', 'hourly', 'kW'], ['Energy Saved by Block', 'saved', 'kWh'], ['Top Room Energy Today', 'roomsChart', 'kWh']].map(item => `<div class="card chart-small"><div class="chart-heading"><h2>${item[0]}</h2><span>${item[2]}</span></div><div class="canvas-wrap"><canvas id="${item[1]}"></canvas></div></div>`).join('')}</div><div class="layout energy-detail"><div class="card"><div class="section-title"><h2>Live Insights</h2><span class="sub">Backend-derived</span></div><div id="energyInsights" class="metric-list"><p class="muted">Waiting for backend data...</p></div></div><div class="card"><div class="section-title"><h2>Energy Waste</h2><span class="sub">Active backend alerts</span></div><div id="energyWaste"><p class="muted">Waiting for backend data...</p></div></div></div>`);
